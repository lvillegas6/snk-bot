...
export class SnkDatabase {
  ...
  public getPlayerManagers(): SnkPlayerManager[] {
    let array: SnkPlayerManager[] = [];
    let guilds = this.getLowdb().get("guilds").value();
    for (var key in guilds)
      array.push(this.getPlayerManager(key));
    return array;
  }
...
export class SnkPlayerManager {
  ...
  public getPlayers(): SnkPlayer[] { // Devuelve todos los jugadores de esta guild
    let array: SnkPlayer[] = [];
    let players = this.get().getLowdb().get("guilds." + this.guildid + ".players").value();
    for (var key in players)
      array.push(this.getPlayer(key));
    return array;
  }
